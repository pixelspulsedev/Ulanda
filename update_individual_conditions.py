#!/usr/bin/env python3
"""Update all condition images - both hero and subcondition thumbnails."""
import re
...
# Individual condition images (thumb + hero = same URL)
individual_conditions = {
    ...
    # Skin texture decline (note: might be duplicate with uneven-texture)
    ...
    # Rough uneven texture (might be uneven-texture)
...
# Use regex to find and replace all image/placeholderUrl pairs
# Pattern: link: '/conditions/.../condition-name'
# followed by image and placeholderUrl

for condition_id, new_url in individual_conditions.items():
    # Pattern to find the link line
    pattern = rf"(link: '/conditions/[^']*/{condition_id}'[^}}]*?image: )\"[^\"]+\"(,\s*placeholderUrl: )\"[^\"]+\""
    
    replacement = rf'\1"{new_url}"\2"{new_url}"'
...
print(f"✓ Updated {len(individual_conditions)} individual condition images")
