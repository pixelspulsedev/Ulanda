#!/usr/bin/env python3
"""Update individual condition detail page images."""
import re
...
# Individual condition images (both hero image and placeholderUrl)
...
# Use regex to find and replace hero images
# Pattern: id: 'condition-name' ... hero: { ... image: "...", placeholderUrl: "..."

count = 0
for condition_id, new_url in individual_conditions.items():
    # Pattern to find the condition block starting with id and then finding the hero image
    pattern = rf"(id:\s*'{condition_id}'[^{{]*?hero:\s*\{{[^}}]*?image:\s*)\"[^\"]+\"(,\s*placeholderUrl:\s*)\"[^\"]+\""
...
print(f"✓ Updated {count} individual condition detail page hero images")
